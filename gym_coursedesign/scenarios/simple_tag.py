import numpy as np
from gym_coursedesign.core import World, Agent, Landmark, Border, Check
from gym_coursedesign.scenario import BaseScenario
import math
...
class Scenario(BaseScenario):
    ...
    def check_world_create(self,world):
        #检测landmark和agent、check是否重合
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                if self.is_collision(landmark, world.check[0]):
                    return False
        ...
        for i,agent in enumerate(world.agents):
            if self.is_collision(agent, world.check[0]):
                return False
        #检测agent之间是否重合
        for i,agent in enumerate(world.agents):
            for j,agent_ in enumerate(world.agents):
                if i != j:
                    if self.is_collision(agent, agent_):
                        return False
        return True
    ...
    def reset_world(self, world, agent_pos=None):
        ...
        #
        # world.check[0].state.p_pos = np.array([-0.5, -0.5])
        # world.check[0].state.p_vel = np.zeros(world.dim_p)

        world.check[0].state.p_pos = 2 * np.random.uniform(0, 0.35, world.dim_p) - 0.35
        world.check[0].state.p_vel = np.zeros(world.dim_p)
        ...
        for agent in world.agents:
            ...
            # agent.state.p_pos = np.random.uniform(-0.28, -0.05, world.dim_p) if agent.adversary else np.random.uniform(0.05, 0.28, world.dim_p)
            if agent_pos is None:
                agent.state.p_pos = 2 * np.random.uniform(0, 0.825,world.dim_p) - 0.825
            else:
                agent.state.p_pos = agent_pos[0] if agent.adversary else agent_pos[1]
            agent.state.p_vel = np.zeros(world.dim_p)  # agent初始速度
            agent.state.c = np.zeros(world.dim_c)  # agent初始交流状态

        # 保证agent、check和landmark不重合
        while not self.check_world_create(world):
            world.check[0].state.p_pos = 2 * np.random.uniform(0, 0.35, world.dim_p) - 0.35
            for agent in world.agents:
                agent.state.p_pos = 2 * np.random.uniform(0, 0.825, world.dim_p) - 0.825
